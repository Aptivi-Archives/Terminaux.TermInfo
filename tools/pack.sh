...
#   Terminaux.TermInfo  Copyright (C) 2023  Aptivi
# 
...
# Pack binary
echo Packing binary...
cd "../src/Terminaux.TermInfo/bin/$releaseconf/netstandard2.0/" && "$zippath" -r /tmp/$version-bin.zip . && cd -
cd "../src/Terminaux.TermInfo.Cli/bin/$releaseconf/net8.0/" && "$zippath" -r /tmp/$version-demo.zip . && cd -
if [ ! $? == 0 ]; then
