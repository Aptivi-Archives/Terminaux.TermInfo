...
"$dotnetpath" msbuild "../src/Terminaux.TermInfo.sln" -t:restore -p:Configuration=$releaseconf
if [ ! $? == 0 ]; then
...
# Build KS
echo Building KS...
"$dotnetpath" msbuild "../src/Terminaux.TermInfo.sln" -p:Configuration=$releaseconf
if [ ! $? == 0 ]; then
	echo Build failed.
	exit 1
fi
